...
void transmit(PacketBuffer * txPkt)
{
	...
	U2TXREG = ESCAPE_CODE;  // first byte of packet, has to be sent from here, as bootstrap
	txByte = 1;
...
void handleTx(PacketBuffer * TxPacket)
{
	...
	switch (txByte) 
	{
		case 1: U2TXREG = HEADER_CODE; break;      // second byte of pkt, first had to be sent in transmit()
		case 2: U2TXREG = TxPacket->chksum; break;
		default:
			if (txByte < TxPacket->length + OVERHEAD) {
				U2TXREG = TxPacket->data[txByte - OVERHEAD];
			} else {
				switch (txByte - (TxPacket->length + OVERHEAD)) {
				case 0: U2TXREG = ESCAPE_CODE; break;
				case 1: U2TXREG = END_CODE; break;     // last byte of pkt
				default:
					TxPacket->length = 0;
					TxPacket->done = 1;
					__asm__ volatile ("DISI #0x1FFF"); //disable interrupts
					IEC1bits.U2TXIE = 0;
					DISICNT = 0; //enable interrupts
					//U2MODEbits.UARTEN = 1; do we need this? enables uart when it is already enabled
				}
			}
	}
	txByte++;
