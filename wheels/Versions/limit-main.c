/* limit-main.c
 *
 * This version of the wheels code limits the amount by which the speed can
 * change for a given command to MAX_CHANGE. So if the robot's current speed
 * were 12 and MAX_CHANGE were set to 5, the sequence of commands
...
// Stuff for the limited change
#define MAX_CHANGE 1	// The most that we can change our speed per command
		...
		if (RxPacket.done) {
			...
			switch(RxPacket.port) {
				case 'r':
					Reset();
				case 'w':
					// Get the transmitted wheel speed and determine the 
					// difference
					newSpeed = RxPacket.data[wheel];
	
					difference = newSpeed - command;
					
					// If the difference between that and the current speed
					// is small enough, just set command to newSpeed. Also
					// incorporate the special case for 0
