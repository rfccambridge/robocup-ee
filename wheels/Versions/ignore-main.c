/* ignore-main.c
 *
 * This version of the wheels main loop ignores commands that flip the sign
 * of the wheel speed unless SPEED_THRESHOLD in a row have the same sign. 
 * So if the robot received a sequence of speeds 
 *                  50 45 47 -30 42 12 -3 -5 -7 10
 * the -30, -3, and 10 would all be ignored. 0 is a special case and is 
 * treated as either sign.
 */
...
// Stuff for ignoring things
#define SPEED_THRESHOLD 2	// Number of commands we must receive before we
							// stop ignoring them
		...
		if (RxPacket.done) {
			...
			switch(RxPacket.port) {
				...
				case 'w':
					...
					/* If not, see whether the sign has been different long
					   enough */
